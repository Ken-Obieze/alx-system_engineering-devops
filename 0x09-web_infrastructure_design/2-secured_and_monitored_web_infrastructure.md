## Firewall
The firewalls protect networks from unwanted and unauthorized users by filtering incoming traffic and dropping packets from suspicious origins or files that do whose checksum is inaccurate.

## SSL.
The SSL certificate is for encrypting the traffic between the web servers and the external network to prevent man-in-the-middle attacks (MITM) and network sniffers from sniffing the traffic which could expose valuable information. The SSL certs ensure privacy, integrity, and identification. If an attacker captures HTTPS packet, they would have a hard time deciphering its content because they do not possess the encription key.
